...
def run(cfg):
   ...
   # get read-per-MT metrics, for all MTs regarless of primer
   metricVals = getRpmtMetrics(mtListAllPrimers)

   # write the metrics for later concatenation
   fileout = open(readSet + ".sum.primer.umis.summary.txt", "w")
   for iRow in range(len(metricVals)):
      fileout.write(str(metricVals[iRow]) + "\t" + metricNames[iRow] + "\n")
   fileout.close()
